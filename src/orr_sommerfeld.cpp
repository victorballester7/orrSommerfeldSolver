...
void OSSolver::mapToStandardRegion(CustomU &Uprofile) {
  ...
  double a = Uprofile.x_data[0];
  double b = Uprofile.x_data[Uprofile.x_data.size() - 1];
...
double OSSolver::getL(uint n, double x) {
  if (n == 0)
    return 1.0;
  if (n == 1)
    return x;
...
double OSSolver::getN(uint i, double z) {
...
double OSSolver::getdN(uint i, double z) {
  assert(i >= 1 && i <= dimVS); // Ensure i is within valid range
  double factor = sqrt((2 * i + 3) * 2.0);
...
double OSSolver::getd2N(uint i, double z) {
  assert(i >= 1 && i <= dimVS); // Ensure i is within valid range
...
complex OSSolver::Los(uint i, uint j) {
...
complex OSSolver::M(uint i, uint j) {
...
complex OSSolver::R0(uint i, uint j) {
...
complex OSSolver::R1(uint i, uint j) {
  // Implementation based on equation (16) in the paper
  complex T4ij = 0.0, T5ij = 0.0, T6ij = 0.0;
...
complex OSSolver::R2(uint i, uint j) {
  // Implementation based on equation (16) in the paper
...
std::vector<complex> OSSolver::solve() {
  // invert matrix B
  Eigen::FullPivLU<Matrix> lu(B);
  ...
  // Extract eigenvalues
  Vector eigenvals = eig.eigenvalues();

  std::vector<complex> result(eigenvals.data(),
                              eigenvals.data() + eigenvals.size());
